...
// Temporary local variables
  ...
  for (int k=0; k<16; k++) {                                              // For each of the LED's in the strand, set a brightness based on a wave as follows.
    ...
    leds[matrice[0][k]] = ColorFromPalette( currentPalette, colorIndex+30, thisbright, currentBlending);   // Let's now add the foreground colour.
    leds[matrice[1][k]] = ColorFromPalette( currentPalette, colorIndex+15, thisbright, currentBlending);
    leds[matrice[2][k]] = ColorFromPalette( currentPalette, colorIndex, thisbright, currentBlending);
        }
